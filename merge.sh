...
cp /usr/local/bin/uhd_black.png "$PWD/uhd_black.png"
...
for file in *.jpg; do
  ...
  a=$(identify -format "w=%w;h=%h" $file)
    ...
    if [ "$w" -ge "$h" ]
    then
    convert "$PWD/$file" -resize x2160 -quality 100 "$filename_resized"
    else
    convert "$PWD/$file" -resize x2160 -quality 100 "$filename_resized"
    fi
  ...
  convert "$PWD/uhd_black.png" "$filename_resized" -gravity center -compose over -composite "$filename_converted"
done
